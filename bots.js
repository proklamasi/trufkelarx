...
async function createBot(botName) {
    ...
    try {
        ...
        function getValidCard(bot, firstPlayedSuit) {
            ...
            if (!firstPlayedSuit) {
                // Only bid winner or round winner can lead (Penambahan)
                if (!bot.isBidWinner && !bot.isRoundWinner) {
                    console.log(`${bot.name}: Not bid/round winner, cannot lead`);
                    return null;
                }

                console.log(`${bot.name}: Leading play as ${bot.isBidWinner ? 'bid winner' : 'round winner'}`);
        ...
        socket.on('updateCurrentTurn', debounce(async (currentTurnId) => {
            ...
            if (bot.currentPhase === 'playing1-phase') {
                ...
                // coba perubahan
                // Replace the nextPlayer calculation
                const nextIndex = (myIndex + 1) % bot.players.length;
                const nextPlayer = myIndex === 3 ? null : bot.players[nextIndex];
                ...
                if ((bot.isBidWinner || nextPlayer) && !bot.playedThisTrick) {
                    try {
                        ...
                        async function isCardEnabled(bot, cardData) {
                            const enabled = await bot.page.evaluate((data) => {
                                const cards = Array.from(document.querySelectorAll('.card'));
                                const targetCard = cards.find(card => {
                                    const cardValue = card.src.match(/(\w+)_of_/)?.[1]?.toLowerCase() || '';
                                    const cardSuit = card.getAttribute('data-suit');
                                    return cardSuit === data.suit && 
                                        cardValue === data.value.toLowerCase();
                                });
                                
                                // Check if card exists and is not disabled
                                return targetCard && !targetCard.classList.contains('disabled');
                            }, cardData);
                            
                            return enabled;
                        }

                        // Modify the click logic in updateCurrentTurn
                        if (cardToPlay && !bot.playedThisTrick) {
                            // Check if card is enabled before attempting to click
                            const isEnabled = await isCardEnabled(bot, cardToPlay);
                            
                            if (!isEnabled) {
                                console.log(`${bot.name}: Card ${cardToPlay.suit} ${cardToPlay.value} is disabled, skipping...`);
                                return;
                            }

                            const clicked = await bot.page.evaluate((cardData) => {
                                const cards = Array.from(document.querySelectorAll('.card:not(.disabled)')); // Only get enabled cards
                                const targetCard = cards.find(card => {
                                    const cardValue = card.src.match(/(\w+)_of_/)?.[1]?.toLowerCase() || '';
                                    const cardSuit = card.getAttribute('data-suit');
                                    return cardSuit === cardData.suit && 
                                        cardValue === cardData.value.toLowerCase();
                                });
