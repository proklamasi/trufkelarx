...
async function createBot(botName) {
    ...
    try {
        ...
        // Helper function to get the lowest-value card
        ...
        socket.on('updateCurrentTurn', async (currentTurnId) => {
            // Debounce turn updates
            const now = Date.now();
            if (bot.lastTurnUpdate && (now - bot.lastTurnUpdate) < 500) {
                console.log(`${bot.name}: Ignoring duplicate turn update`);
                return;
            }
            bot.lastTurnUpdate = now;
        
            if (bot.currentPhase === 'playing1-phase') {

                console.log(arePilesEmpty(page));
                // Always check players order
                const myIndex = bot.players.indexOf(bot.name);
                console.log(myIndex, bot.players);
        
                // Check if it's first play of the trick
                const pilesEmpty = await arePilesEmpty(page);
        
                // Modify shouldPlay condition in updateCurrentTurn
                const shouldPlay = (
                    // First play of first trick - bid winner leads
                    (bot.isBidWinner && pilesEmpty) || 
                    // First play of subsequent tricks - round winner leads
                    (bot.isRoundWinner && pilesEmpty) ||
                    // Following plays - check player order
                    (!bot.isBidWinner && !bot.isRoundWinner && bot.players[myIndex] === bot.name)
                ) && !bot.playedThisTrick;
                ...
                if (shouldPlay) {
                    try {
                        ...
                        // Add slight delay before action
                        await new Promise(resolve => setTimeout(resolve, 300));
        
                        if (!bot.isBidWinner) {
                            // Wait for first card if not bid winner
                            firstPlayedCard = await waitForFirstPlayedCard(page);
                            if (!firstPlayedCard) {
                                console.log(`${bot.name}: Waiting for lead card...`);
                                return;
                            }
                        }
        
                        // Get valid card to play
                        const firstPlayedSuit = firstPlayedCard ? firstPlayedCard.suit : null;
                        const cardToPlay = getValidCard(bot, firstPlayedSuit);
        
                        if (cardToPlay) {
                            // Update the card clicking logic in updateCurrentTurn handler
                            const clicked = await page.evaluate((cardData) => {
                                // Only get cards that are clickable (not disabled)
                                const cards = Array.from(document.querySelectorAll('#playerHand .card:not(.disabled)'));
                                
                                // Debug clickable cards
                                console.log('Clickable cards:', cards.map(c => ({
                                    suit: c.getAttribute('data-suit'),
                                    value: c.src.match(/(\w+)_of_/)?.[1] || '',
                                    disabled: c.classList.contains('disabled')
                                })));
                            
                                // Find our target card among clickable cards
                                const targetCard = cards.find(card => {
                                ...
                                if (targetCard) {
                                    // Card is found and clickable
                                    targetCard.click();
                                    return true;
                                }
                                
                                // Either card not found or not clickable
                                console.log(`Card ${cardData.suit} ${cardData.value} not clickable`);
                                return false;
                            }, cardToPlay);
        
                            if (clicked) {
                                // Update state and emit event only after successful click
                                bot.playedThisTrick = true;
                                console.log(`${bot.name}: ${bot.isBidWinner ? 'Led with' : 'Played'} ${cardToPlay.suit} ${cardToPlay.value}`);
                            } else {
                    ...
                    console.log(`${bot.name}: Not playing:`, {
                        isBidWinner: bot.isBidWinner,
                        pilesEmpty: pilesEmpty,
                        playedThisTrick: bot.playedThisTrick,
                        myTurn: bot.players[myIndex] === bot.name
                    });
                }
            }
        });
